...
そしてpush_forward(th)を実行する。これは単にfp[th]++　bp[th]--をするだけである。
pushするとともにpop_backをする。もどりつつfp++するだけだ。なぜならデータは保存されており逆順にするだけだから。
再試行する場合にはこのfp[th]を参照する。ここが0である場合には
通常のnondetの処理となる。空でない場合にはpop_forwardにより直前の
choiceを得てgotoで飛ぶ。この場合にはunifyはスキップする。要するにやりたいことは
C言語のスタックを再現することである。fpが底をついたときが直前の状態である。
arglistを回復させて次の節を実行する。これを全部失敗するまで繰り返す。
つまりBack_stackを一時不能、再度可能にすることで再試行に
ともなう再突入を可能にするものである。データはコピーする必要はない。back_stackに
破壊されずに保持されているからだ。popするときに再突入のためのbiasに9999をいれる。
...
Q :- R.
R :- S.
Pの呼び出しにおいてSでバックトラックが生じるかもしれない。このときには
上記の方法が役に立つ。再帰以外の場合においても=YESで成功したときには
push_forward(th)をしておく。Pにおいてバックトラックが生じる場合があるからだ。
この場合にはPを入口として再突入したあとpop_forwardする。
再帰の場合と異なり成功時のarglistを保存する必要はない。
push_forward_notrec(th)として区別する。

P :- Q,P.
このように再帰する場合にはQをバックトラックすることはない。
したがって再帰実行の前にfp[th]=0とすることで再帰呼出しのP
は通常の動作となる。
再帰呼出しでバックトラックになるのはappend/3のようなケースに限られる。
P.
P :- P. 
この場合に限られるはずだ。

再々考察
forwardは複数の述語にものが積み上がる場合がある。
