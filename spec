...
prepare: selfstackに現時点でのwp,sp,acを記憶する。これはインタプリタで利用する。
         ...
         prepareはarityの引数アドレス、argsの配列アドレスを受取ここに値を返すようにする。
         lengthやnthを毎回呼び出すのはコストが大きい。一度だけしらべてbackstackに保存
         しておく。こうすれば失敗により再度呼び出されたときに効率がいい。nondetのときに
         これが有効に機能するはずだ。受け取るarglist、出力する&n、args[]の引数を追加する。
...
incchoice：releaseでは節選択点は変更しない。rest==NIL のときに選択点を＋１する。
