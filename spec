...
にはBPをpopする。バックトラックに使ってきたbackstackを初期化する。ARGLIST_BACKSTACKインデックスを0とする。

push_back：非決定性述語を呼び出すときに呼び出し元で生成、backstackにpushしておく。
ここにwp,sp,acが保存されChoice節は0に初期化される。
bp[th]がすでに利用されている場合は、それはバックトラックにより再突入することを意味する。
ARGLIST_BACKSTACKがUNBINDあるいは特定のarglistである場合にはバックトラックにより後戻りした経路である。
その場合にはそれに従いSKIPにより再突入する
ARGLIST_BACKSTACKがUNBINDでない場合にはそれを返す。そうでなければ与えられたarglistを返す。

push_rec: 非決定性述語の再帰呼出しの場合につかう。BACKSTACKは連言方向と、再帰方向の2次元的
配列で管理する。
...
R    bp[3,1,th]
     Q1         Qrec2      Qrec3       
Q    bp[2,1,th] bp[2,2,th] bp[2,3,th] ->
P    bp[1 ,1,th]



バックトラックして述語に再突入するときには次のswitchのSKIPによりunifyをスキップして
直前の位置までもどる。SPなどの値はそのときの値がBACKSTACKに保持されている。
pushbackは保持されているARGLISTがあればそれを返す。
switch(clause){
    case 0: goto clause_A_M_N_0;
    case 10000: goto skip_A_M_N_0;
    case 1: goto clause_A_M_N_1;
    case 10001: goto skip_A_M_N_1;
    default: goto allfail;
}

C言語においては関数呼び出しのスタックを取り出すことができない。したがって
再度前回の成功した経路をたどることによってスタックを復元しているのである。
ここがWAMと大きく異なる点だ。
...
get_back_choice: bpによりchoice番号をえる。これによりどの節に飛ぶのかが決まる。
バックトラックによりもどった経路に再突入する場合にはchoice＋9999を返す。SKIPに利用される。
そうでなければchoiceをかえす。ARGLIST_BACKSTACKが0かあるいはUNBIND、特定のセルかどうかで
9999を足すかどうかを決める。
