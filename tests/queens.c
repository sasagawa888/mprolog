...
{
skip_3_1:;
int arg_3_1_0 = Jwlistcons(varQ,Jwlistcons(varH,Jwlistcons(varT,Jwlistcons(varL1,NIL,th),th),th),th);
Jpush_recur(th);
retry_3_1_0:;
if (c_qdelete(arg_3_1_0,NIL,th) == YES){
Jpop_recur(th);
Jpush_recur(th);
if (Jcall_det(Jmakecomp("nodiag"),Jwlistcons(varHistory,Jwlistcons(varQ,Jwlistcons(Jmakeint(1),NIL,th),th),th),th) == YES){
Jpop_recur(th);
int arg_3_1_2 = Jwlistcons(varL1,Jwlistcons(Jwlistcons(varQ,varHistory,th),Jwlistcons(varM,NIL,th),th),th);
Jpush_recur(th);
retry_3_1_2:;
if (c_queen_2(arg_3_1_2,NIL,th) == YES){
Jpop_recur(th);
return(YES);
}else{Jset_mode(RETRY,th);
goto retry_3_1_0;}
